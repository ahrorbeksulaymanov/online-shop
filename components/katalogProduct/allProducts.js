...
const AllProducs = () => {
    ...
    useEffect(() => {
        if (page == 1) {
            setAllData(allProduct.slice(0, 8))
        } else if (page == 2) {
            setAllData(allProduct.slice(8, 15))
        }
    }, [page])
            ...
            <div className="slider_my">
                <div className="row">
                                    ...
                                    item?.skidka !== 0 ?
                                        <Badge.Ribbon text={item.skidka + " %"} color="#ff4d4f">
                                            ...
                                            >
                                                ...
                                                <Button style={{ borderRadius: "0" }} disabled={products?.includes(item.id)} onClick={() => addToCard(item.id, item)} className="w-100">{products?.includes(item.id) ? "Added to card" : "Add to card"} <MdAddShoppingCart className="ms-2" /></Button>
                                            </Card>
                                        </Badge.Ribbon>
                                        : <Card
                                            hoverable
                    ...
                    <div className="me-1 mt-3 d-flex justify-content-end">
                        <Pagination current={page} onChange={changePage} total={allProduct.length} />
                    </div>
                </div>
            </div>
        </Styles>
