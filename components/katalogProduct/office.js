...
const OfficeProducts = () => {
    ...
    const [allData, setAllData] = useState(office)
    const [page, setpage] = useState(1)
    const dispatch = useDispatch()

    useEffect(() => {
        if (page == 1) {
            setAllData(office.slice(0, 8))
        } else if (page == 2) {
            setAllData(office.slice(8, 15))
        }
    }, [page])
            ...
            <div className="slider_my">
                <div className="row">
                    ...
                    <div className="me-1 mt-3 d-flex justify-content-end">
                        <Pagination current={page} onChange={changePage} total={office.length} />
                    </div>
                </div>
            </div>
        </Styles>
